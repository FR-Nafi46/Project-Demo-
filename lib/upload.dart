...
class _UploadPageState extends State<UploadPage> {
  ...
  Future<String?> _uploadSingleImage(Uint8List bytes, String uid, int order) async {
    final fileName = '$uid/${DateTime.now().millisecondsSinceEpoch}_$order.jpg';
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_selectedImages.isNotEmpty) {
        final coverBytes =
        _selectedImages[_coverIndex]['bytes'] as Uint8List;
        final uploaded = await _uploadSingleImage(coverBytes, uid, 0);
      ...
      // Upload all images and insert into product_images
      for (int i = 0; i < _selectedImages.length; i++) {
        final bytes = _selectedImages[i]['bytes'] as Uint8List;
        final isCover = i == _coverIndex;

        // Cover was already uploaded above — reuse its URL
        String url;
        if (isCover) {
          url = coverUrl;
        } else {
          url = (await _uploadSingleImage(bytes, uid, i)) ?? coverUrl;
        }
