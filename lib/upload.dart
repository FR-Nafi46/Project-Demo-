...
class _UploadPageState extends State<UploadPage> {
  ...
  Future<String?> _uploadProductImage(Uint8List bytes, String uid) async {
  ...
  /// Converts LatLng to a human-readable address using all available fields
  ...
  /// Formats a Placemark into the most detailed address possible
  /// Examples: "Amborkhana, Sylhet 3100, Bangladesh" or "25/A Modina Market, Sylhet"
  String _formatDetailedAddress(Placemark place) {
    List<String> parts = [];

    // Street number + thoroughfare (e.g., "25/A Modina Market")
    String street = '';
    ...
    // SubLocality (e.g., "Amborkhana")
    if (place.subLocality != null && place.subLocality!.isNotEmpty) {
      parts.add(place.subLocality!);
    }
    // Locality (city, e.g., "Sylhet")
    ...
    // Administrative area (state/division, e.g., "Sylhet Division")
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_selectedImageBytes != null) {
        final uploaded = await _uploadProductImage(_selectedImageBytes!, uid);
...
// ─────────────────────────────────────────────────────────────
// Location Picker Map Widget (full screen map picker)
// ─────────────────────────────────────────────────────────────
...
class _LocationPickerMapState extends State<LocationPickerMap> {
  ...
  /// Converts LatLng to a detailed address (street, area, city, country)
  Future<void> _getAddressFromLatLng(LatLng position) async {
    ...
    try {
      ...
      if (placemarks.isNotEmpty) {
        Placemark place = placemarks[0];
        String address = _formatDetailedAddress(place);
        setState(() {
  ...
  String _formatDetailedAddress(Placemark place) {
    ...
    // Administrative area (state/division)
    if (place.administrativeArea != null && place.administrativeArea!.isNotEmpty) {
      parts.add(place.administrativeArea!);
    }
    // Country
