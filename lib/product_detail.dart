...
// ─────────────────────────────────────────────────────────────────────────────
// Full-screen image viewer (reusable)
// ─────────────────────────────────────────────────────────────────────────────
...
class _ProductDetailPageState extends State<ProductDetailPage> {
              ...
              Expanded(
                child: ElevatedButton(
                  onPressed: () {
                    ...
                    Navigator.push(
                        context, MaterialPageRoute(builder: (_) => const MeritHistoryPage()));
                  },
  ...
  Future<void> _chatWithSeller() async {
    ...
    try {
      final convId =
      await SupabaseService.getOrCreateConversation(uid, sellerId, widget.product.id);
      if (mounted) {
        Navigator.push(
            context,
            MaterialPageRoute(
  ...
  /// Opens the product image full-screen
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: CustomScrollView(
        slivers: [
          // ── Hero image with tap-to-fullscreen ──────────────────────────────
          SliverAppBar(
            expandedHeight: 300,
            pinned: true,
            backgroundColor: primaryColor,
