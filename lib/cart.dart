  ...
  Future<void> _checkout() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: Text(
            'Confirm your order of ${cartItems.length} item(s) for a total of ৳${total.toStringAsFixed(2)}?'),
        actions: [
          TextButton(
    ...
    // Fetch buyer's name once
    final buyerProfile = await SupabaseService.fetchProfile(uid);
    final buyerName =
        (buyerProfile?['full_name'] as String?) ?? 'A user';

    // Notify each product owner
    for (final item in cartItems) {
      final ownerId = item.ownerId;
      if (ownerId == null || ownerId == uid) continue; // skip own products

      await SupabaseService.notifyCartInterest(
        productOwnerId: ownerId,
        buyerName: buyerName,
        productName: item.name,
        productId: item.productId,
        isRent: item.isRental,
      );
    }
    ...
    if (mounted) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
        content: Text(
            '✅ Order confirmed! Product owners have been notified.'),
