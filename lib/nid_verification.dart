...
class _NIDVerificationPageState extends State<NIDVerificationPage> {
  ...
  Future<void> _submitVerification() async {
    ...
    try {
      await SupabaseService.uploadNIDImages(uid, _frontImage!, _backImage!);

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Verification request submitted! Admin will review shortly.')),
        );
          ...
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: _isVerified ? Colors.green.shade50 : (_hasRequested ? Colors.orange.shade50 : Colors.grey.shade50),
                borderRadius: BorderRadius.circular(16),
                border: Border.all(
                  color: _isVerified ? Colors.green : (_hasRequested ? Colors.orange : Colors.grey),
                  width: 1,
                ),
              ),
              child: Row(
                children: [
                  Icon(
                    _isVerified ? Icons.verified : (_hasRequested ? Icons.pending : Icons.warning),
                    color: _isVerified ? Colors.green : (_hasRequested ? Colors.orange : Colors.grey),
                    size: 32,
                  ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          _isVerified ? 'Verified Account' : (_hasRequested ? 'Verification Pending' : 'Not Verified'),
                          style: TextStyle(
                            fontSize: 16,
                            fontWeight: FontWeight.bold,
                            color: _isVerified ? Colors.green : (_hasRequested ? Colors.orange : Colors.grey.shade700),
                          ),
                        ),
