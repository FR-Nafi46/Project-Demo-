          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                ...
                Center(child: Text(_isLogin ? 'Welcome Back' : 'Create Account',
                    style: const TextStyle(fontSize: 28, fontWeight: FontWeight.bold))),
                Center(child: Text(_isLogin ? 'Sign in to your account' : 'Join RentalApp today',
                    ...
                    child: RichText(
                      text: TextSpan(
                        ...
                        children: [
                          ...
                          TextSpan(text: _isLogin ? 'Register' : 'Log In',
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  Future<void> _loadProfile() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _user = profile != null
              ? AppUser.fromMap(uid, email, profile)
              : AppUser(id: uid, email: email, fullName: email.split('@')[0]);
          _merit = merit;
  ...
  Future<void> _pickAndUploadAvatar() async {
    ...
    try {
      final String fileName = 'avatars/$uid/${DateTime.now().millisecondsSinceEpoch}.jpg';
      ...
      publicUrl = Supabase.instance.client.storage.from('avatars').getPublicUrl(fileName);
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
                ...
                children: [
                  Container(
                    decoration: BoxDecoration(
                      ...
                      border: Border.all(color: _isAdmin ? Colors.amber : tierColor, width: 3),
                      boxShadow: [BoxShadow(color: (_isAdmin ? Colors.amber : tierColor).withOpacity(0.3), blurRadius: 16, spreadRadius: 2)],
                    ),
                    child: CircleAvatar(
                      radius: 56,
                      ...
                      child: (_user?.avatarUrl == null || _user!.avatarUrl!.isEmpty)
                          ? const Icon(Icons.person, size: 56, color: Colors.white) : null,
                    ),
                  ),
                  GestureDetector(
                  ...
                  if (_isAdmin) ...[
                    ...
                    Container(
                      ...
                      child: const Row(
                        ...
                        children: [
                          Icon(Icons.admin_panel_settings, size: 14, color: Colors.amber),
                          SizedBox(width: 4),
                          Text('Admin', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.amber)),
                        ],
                      ),
                    ),
                  ],
              ...
              Text(_user?.email ?? '', style: TextStyle(color: Colors.grey.shade600, fontSize: 13)),
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  borderRadius: BorderRadius.circular(20),
                  border: Border.all(color: _isVerified ? Colors.green.shade200 : Colors.orange.shade200),
                ),
                child: Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    ...
                    Text(
                      _isVerified ? 'NID Verified' : 'NID Not Verified',
                      style: TextStyle(
                        fontSize: 11,
              ...
              if (!_isAdmin)
                GestureDetector(
                  onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MeritHistoryPage())),
                  child: Container(
                    ...
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                        Row(children: [
                          Icon(Icons.military_tech, color: tierColor, size: 22),
                          const SizedBox(width: 8),
                          const Text('Merit Score', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold)),
                        ]),
                        Row(children: [
                          Text('${merit.points}/100',
                              style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: tierColor)),
                      ...
                      const SizedBox(height: 10),
                      Text(tier.description, style: TextStyle(fontSize: 12, color: Colors.grey.shade600)),
                      if (!merit.canRent) ...[
                        const SizedBox(height: 10),
                        Container(
                      ...
                      Row(children: [
                        ...
                        const SizedBox(width: 4),
                        Text('Daily gain remaining: +${merit.remainingDailyGain} pts',
                            style: TextStyle(fontSize: 11, color: Colors.grey.shade500)),
                        const Spacer(),
                        Text('Tap for history →', style: TextStyle(fontSize: 11, color: tierColor, fontWeight: FontWeight.w500)),
                      ]),
                    ]),
                  ),
                ),
              ...
              Container(
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: const Icon(Icons.account_balance_wallet, color: Colors.green, size: 22),
                  ),
                  const SizedBox(width: 12),
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Wallet Balance', style: TextStyle(fontSize: 12, color: Colors.grey)),
                    Text('৳${_user?.walletBalance.toStringAsFixed(2) ?? '0.00'}',
              ...
              const Divider(height: 1),
              _MenuItem(icon: Icons.account_circle_outlined, title: 'Edit Profile', onTap: () =>
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const EditProfilePage())).then((_) => _loadProfile())),
              const Divider(height: 1),
              ...
              const Divider(height: 1),
              _MenuItem(icon: Icons.logout, title: 'Log Out', onTap: _logout, color: Colors.red),
              const SizedBox(height: 24),
            ],
          ),
        ),
...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text('Profile updated!'), backgroundColor: Color(0xFF381932)));
        ...
        child: Column(
          ...
          children: [
            ...
            TextFormField(
              controller: _bioCtrl,
              maxLines: 3,
              decoration: _inputDec('A short bio about yourself', Icons.info_outline),
            ),
            const SizedBox(height: 32),
            SizedBox(
              width: double.infinity, height: 54,
  ...
  Widget _field(TextEditingController ctrl, String hint, IconData icon) {
    return TextFormField(controller: ctrl, decoration: _inputDec(hint, icon));
  }
...
class _SettingsPageState extends State<SettingsPage> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _settingsCard([
              _navTile(Icons.account_circle_outlined, 'Edit Profile', onTap: () =>
                  Navigator.push(context, MaterialPageRoute(builder: (_) => const EditProfilePage()))),
              _divider(),
              _navTile(Icons.lock_outline, 'Change Password', onTap: () => _showChangePasswordSheet()),
              _divider(),
              _navTile(Icons.email_outlined, 'Change Email', onTap: () {}),
              _divider(),
              _navTile(Icons.phone_outlined, 'Phone Number', onTap: () {}),
            ]),
            const SizedBox(height: 16),
            ...
            _settingsCard([
              _toggleTile(Icons.notifications_outlined, 'Push Notifications', _pushNotifications,
                      (v) => setState(() => _pushNotifications = v)),
              _divider(),
              _toggleTile(Icons.email_outlined, 'Email Notifications', _emailNotifications,
                      (v) => setState(() => _emailNotifications = v)),
              _divider(),
              _toggleTile(Icons.schedule, 'Rental Reminders', _rentalReminders,
                      (v) => setState(() => _rentalReminders = v)),
              _divider(),
              _toggleTile(Icons.chat_bubble_outline, 'Chat Notifications', _chatNotifications,
                      (v) => setState(() => _chatNotifications = v)),
              _divider(),
              _toggleTile(Icons.military_tech, 'Merit Alerts', _meritAlerts,
            ...
            _settingsCard([
              _toggleTile(Icons.dark_mode_outlined, 'Dark Mode', _darkMode,
              ...
              ListTile(
                ...
                trailing: Row(mainAxisSize: MainAxisSize.min, children: [
                  Text(_language, style: TextStyle(color: Colors.grey.shade600)),
            ...
            _settingsCard([
              _navTile(Icons.visibility_outlined, 'Profile Visibility', onTap: () {}),
              _divider(),
              _navTile(Icons.security, 'Two-Factor Authentication', onTap: () {}),
              _divider(),
              _navTile(Icons.block, 'Blocked Users', onTap: () {}),
              _divider(),
              _navTile(Icons.download_outlined, 'Download My Data', onTap: () {}),
            ]),
            const SizedBox(height: 16),
            ...
            _settingsCard([
              _navTile(Icons.help_outline, 'Help Center', onTap: () {}),
              _divider(),
              _navTile(Icons.report_outlined, 'Report a Problem', onTap: () {}),
              _divider(),
              _navTile(Icons.star_border, 'Rate the App', onTap: () {}),
              _divider(),
              _navTile(Icons.info_outline, 'About RentalApp', onTap: () => _showAboutDialog()),
            ]),
            const SizedBox(height: 16),
            ...
            _settingsCard([
              _navTile(Icons.logout, 'Log Out', color: Colors.red, onTap: () async {
                await SupabaseService.signOut();
                if (mounted) {
              ...
              _divider(),
              _navTile(Icons.delete_forever_outlined, 'Delete Account', color: Colors.red, onTap: () => _showDeleteAccountDialog()),
            ]),
            const SizedBox(height: 32),

            Center(child: Text('RentalApp v1.0.0', style: TextStyle(color: Colors.grey.shade400, fontSize: 12))),
            const SizedBox(height: 16),
          ],
        ),
      ),
  ...
  Widget _toggleTile(IconData icon, String title, bool value, ValueChanged<bool> onChanged) {
    return ListTile(
      leading: Icon(icon, color: const Color(0xFF381932)),
      title: Text(title),
      trailing: Switch(value: value, onChanged: onChanged, activeColor: const Color(0xFF381932)),
    );
  }

  Widget _navTile(IconData icon, String title, {VoidCallback? onTap, Color? color}) {
    return ListTile(
      leading: Icon(icon, color: color ?? const Color(0xFF381932)),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              onPressed: () {
                ...
                Navigator.pop(context);
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Password update requested — check your email.')));
              },
              style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF381932), foregroundColor: Colors.white,
    ...
    showModalBottomSheet(
      ...
      builder: (_) => Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        ...languages.map((l) => ListTile(
          title: Text(l),
          trailing: _language == l ? const Icon(Icons.check, color: Color(0xFF381932)) : null,
  ...
  void _showAboutDialog() {
    showAboutDialog(
      ...
      applicationIcon: const Icon(Icons.home_work, size: 40, color: Color(0xFF381932)),
      children: [const Text('Rent anything, anywhere.\n\n© 2025 RentalApp. All rights reserved.')],
    );
  }
  ...
  void _showDeleteAccountDialog() {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      content: const Text('This action is irreversible. All your data, listings, and merit history will be permanently deleted.'),
      actions: [
        TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')),
        ElevatedButton(
          onPressed: () { Navigator.pop(context); ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please contact support to delete your account.'))); },
