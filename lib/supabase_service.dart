...
class SupabaseService {
  ...
  /// Sends a password reset email. Supabase emails the user a link that
  /// opens the app (or web) where they can enter a new password.
  ...
  static Future<String> uploadChatImage(String userId, String conversationId, Uint8List bytes) async {
    ...
    return client.storage.from('chat_images').getPublicUrl(fileName);
  }
  ...
  static Future<void> updateRentRequestStatus(String requestId, String status, {DateTime? rentalEndDate}) async {
    ...
    if (rentalEndDate != null) {
      data['rental_end_date'] = rentalEndDate.toIso8601String();
    }
    ...
    if (status == 'accepted' && rentalEndDate != null) {
      ...
      final request = await client.from('rent_requests').select('product_id').eq('id', requestId).single();
  ...
  static Future<void> updateBuyRequestStatus(String requestId, String status) async {
    ...
    if (status == 'accepted') {
      final request = await client.from('buy_requests').select('product_id').eq('id', requestId).single();
