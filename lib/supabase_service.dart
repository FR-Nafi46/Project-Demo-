...
class SupabaseService {
  ...
  static Future<void> uploadNIDImages(
    ...
    // Path format: <uid>/front_<timestamp>.jpg  (no bucket prefix in path)
    ...
    await upsertProfile({
      'id': userId,
      'nid_front_url': frontUrl,   // stores the storage path, e.g. "uid/front_xxx.jpg"
  ...
  /// Uploads NID image bytes to the private 'verification' bucket.
  /// Returns the storage FILE PATH (not a public URL), e.g. "uid/front_123.jpg"
  static Future<String> _uploadNIDImageBytes(
    ...
    // Return the path only — we'll generate signed URLs on demand
    return filePath;
  }

  /// Generates a temporary signed URL for a private NID image.
  /// [storedValue] can be either:
  ///   - a plain path like "uid/front_123.jpg"  (new format)
  ///   - a full public URL (old format — we extract the path from it)
  ...
  /// Extracts the file path inside the 'verification' bucket from a stored value.
  /// Handles both plain paths and full Supabase storage URLs.
  static String _extractNIDPath(String storedValue) {
    ...
    // Full URL format:
    // https://<project>.supabase.co/storage/v1/object/public/verification/<path>
    // or
    // https://<project>.supabase.co/storage/v1/object/sign/verification/<path>?token=...
    final uri = Uri.parse(storedValue);
    final segments = uri.pathSegments;
    // Find the bucket name 'verification' in the path and return everything after it
    final bucketIndex = segments.indexOf('verification');
    if (bucketIndex != -1 && bucketIndex < segments.length - 1) {
      return segments.sublist(bucketIndex + 1).join('/');
    }
    // Fallback: return the stored value as-is and hope for the best
    return storedValue;
  }
  ...
  static Future<void> topUpWallet(String userId, double amount) async {
    ...
    if (wallet == null) {
      ...
      final newBalance =
          (wallet['balance'] as num).toDouble() + amount;
      await client
          .from('wallets')
